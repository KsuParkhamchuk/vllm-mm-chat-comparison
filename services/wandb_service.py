...
def init_wandb(project_name: str = "mm-chat-comparison", run_name: str = None, config: dict = None, job_type: str = "chat_application"):
    """
    ...
    try:
        _run = wandb.init(project=project_name, name=run_name, config=config, job_type=job_type, reinit=True)
        _wandb_initialized = True
        logger.info(f"W&B initialized for project '{project_name}', run_id: {_run.id if _run else 'N/A'}")
...
def log_generation_data(input_tok_s: float = None, output_tok_s: float = None, latency_ms: float = None, **kwargs):
    """
...
def log_vllm_request_output_metrics(vllm_request_output: RequestOutput if RequestOutput else 'typing.Any', manual_duration_sec: float = None):
    ...
    if vllm_request_output.outputs and len(vllm_request_output.outputs) > 0:
        ...
        logger.warning(f"No outputs found in vLLM RequestOutput for request_id: {vllm_request_output.request_id} when calculating token counts.")
    ...
    if manual_duration_sec is not None:
        ...
        if num_generated_tokens > 0 and manual_duration_sec > 0:
            metrics_to_log["manual_output_tok_per_sec"] = num_generated_tokens / manual_duration_sec
    ...
    if vllm_request_output.metrics:
        ...
        if vllm_request_output.outputs and len(vllm_request_output.outputs) > 0:
            ...
            logger.warning(f"No outputs found in vLLM RequestOutput for request_id: {vllm_request_output.request_id}")
        ...
        if metrics.arrival_time is not None:
            if metrics.first_scheduled_time is not None and metrics.first_token_time is not None:
                prompt_processing_time_sec = metrics.first_token_time - metrics.first_scheduled_time

            if metrics.first_token_time is not None:
                time_to_first_token_ms = (metrics.first_token_time - metrics.arrival_time) * 1000
        ...
        if num_prompt_tokens > 0:
            if prompt_processing_time_sec is not None and prompt_processing_time_sec > 0:
        ...
        if hasattr(metrics, 'scheduler_time') and isinstance(metrics.scheduler_time, (int, float)):
        ...
        if metrics_to_log_filtered:
            ...
            logger.info("No valid metrics to log for request_id: %s", vllm_request_output.request_id)

    else:
        # This warning now specifically refers to vLLM's internal detailed metrics
        logger.warning("No internal detailed metrics (vllm_request_output.metrics) found in vLLM RequestOutput for request_id: %s. Manual metrics may still be logged.", vllm_request_output.request_id)
    ...
    if metrics_to_log_filtered:
        ...
        if "request_id" not in metrics_to_log_filtered and hasattr(vllm_request_output, 'request_id'):
        ...
        logger.info("No valid metrics to log after filtering for request_id: %s", vllm_request_output.request_id)
